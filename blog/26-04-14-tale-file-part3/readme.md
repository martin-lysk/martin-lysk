...
# SQLite on Git, Part II: Sqlite reads on git objects
...
In the previous post, we looked behind the curtain of **zlib** the compression library. We managed to recompress a loose object from git - fully git compatible - while keeping separate chunks inside of it acceisble indipendtly.

**The Problem:** Even if a Git object contains a perfectly valid SQLite database file, SQLite cannot simply read from it directly, it expects the content of the sqlite file to be decompressed.


In this article we're going to skretch the surface of Sqlites internals, just enough to build a SQLite VFS (Virtual File System) capable of reading only the parts of the Git blob it actually needs.
...
- [**Part 3**](../26-04-01-tale-file-part2/readme.md): TODO add summary 

TODO add same style of sentence this time refere to the bottle (vfs) "drink me" that grows the blob back into the format sqlite expects
